// =============================================================================
// Apple Root CA — ปักหมุดไว้ในโค้ด ไม่ดาวน์โหลดตอน runtime
//
// ทำไมต้องฝัง: ถ้าไปโหลดตอน runtime แปลว่าเชื่อเครือข่าย ณ ตอนนั้น
//   คนที่แทรกกลางทางเปลี่ยน root ได้ = ปลอมใบเสร็จได้ทั้งระบบ
//   การปักหมุดทำให้ chain ต้องจบที่ Apple จริงเท่านั้น
//
// ปักหมุดเฉพาะ **Apple Root CA - G3** ใบเดียว เพราะเป็นใบเดียวที่ Apple ใช้เป็นราก
// ของ chain สำหรับ signed transaction (StoreKit 2 / App Store Server API)
// เคยฝัง G2 และ Apple Root CA ใบเดิมไว้ด้วย แล้วตัดออก — สองใบนั้นเป็น RSA
// ไม่ได้อยู่ใน chain ของ IAP เลย เก็บไว้มีแต่เพิ่มพื้นที่ที่ต้องไว้ใจโดยไม่จำเป็น
//
// ที่มา: https://www.apple.com/certificateauthority/  (ดาวน์โหลดได้ฟรี ไม่ต้องมีบัญชี)
// ตรวจสอบซ้ำได้ด้วย: openssl x509 -inform DER -in <ไฟล์> -noout -subject -fingerprint -sha256
// =============================================================================


/** Apple Root CA - G3   (ใบที่ใช้เซ็น App Store Server API ปัจจุบัน)
 *  SHA-256 (DER): 63343abfb89a6a03ebb57e9b3f5fa7be7c4f5c756f3017b3a8c488c3653e9179 */
const APPLEROOTCA_G3_B64 =
...
/** Apple Root CA - G2
 *  SHA-256 (DER): c2b9b042dd57830e7d117dac55ac8ae19407d38e41d88f3215bc3a890444a050 */
const APPLEROOTCA_G2_B64 =
...
/** Apple Root CA        (ใบเก่า เก็บไว้เผื่อ chain เดิม)
 *  SHA-256 (DER): b0b1730ecbc7ff4505142c49f1295e6eda6bcaed7e2c68c5be91b5a11001f024 */
const APPLEINCROOTCERTIFICATE_B64 =
...
export const APPLE_ROOT_CAS: Uint8Array[] = [
  Uint8Array.from(atob(APPLEROOTCA_G3_B64), (c) => c.charCodeAt(0)),
]

/** ลายนิ้วมือไว้ตรวจซ้ำตอน startup — ถ้าไม่ตรงแปลว่าไฟล์นี้ถูกแก้ */
export const APPLE_ROOT_SHA256: Record<string, string> = {
